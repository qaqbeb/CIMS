...
public class WarehouseServiceImpl extends ServiceImpl<WarehouseMapper, Warehouse> implements WarehouseService {
    ...
    public Integer warehouseTotalNumber() {
        // 计算库存总量
        List<Warehouse> warehouseList = new ArrayList<>(WHM.selectList(null));
        int totalNum = 0;
        for (Warehouse wh : warehouseList) {
            totalNum += wh.getNum().intValue();
        }
        return totalNum;
    }
    ...
    public Double warehouseTotalPrice() {
        ...
        for (Warehouse wh : warehouseList) {
            String goodsStatement = wh.getGoodsList();
            String goodsList[] = goodsStatement.split("-");
            int len = goodsList.length / 2;
            for (int i = 0; i < len; i++) {
                int goodsId = Integer.parseInt(goodsList[2 * i]);
                int goodsNum = Integer.parseInt(goodsList[2 * i + 1]);
                totalPrice += GM.selectById(goodsId).getPriceIn() * goodsNum;
            }
        }
        return totalPrice;
    }
    ...
    public DataVO<SaleGoodsVO> totalGoodsList() {
        ...
        for (Warehouse wh : warehouseList) {
            str = parseString.mergeStatment(str, wh.getGoodsList());
        }

        HashMap<Goods, Integer> goodsMap = new HashMap<>(parseString.parseToGoodsList(str, GM));

        List<SaleGoodsVO> goodsVOList = new ArrayList<>();

        for (Goods good : goodsMap.keySet()) {
            SaleGoodsVO saleGood = new SaleGoodsVO(good, goodsMap.get(good));
            goodsVOList.add(saleGood);
        }
